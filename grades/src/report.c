...
float interpolatequantile(Freqs *fp, int n, float q) {
  float pq = 0, nq = 0, ps = 0, ns = 0, qdiff = 0, sdiff = 0, s = 0;
...
void reportquantilesummaries(FILE *fd, Stats *s) {
  ...
  // removed float = q not used
  float r = 0.0, min = 0.0, max = 0.0;
  ...
  fprintf(fd, "QUANTILE SUMMARIES\n\n");
        for(csp = s->cstats; csp != NULL; csp = csp->next) {
           ...
           for(i = 0; i < n; i++)
             scores[i] = interpolatequantile(csp->freqs, csp->tallied, quantiles[i]);
           min = scores[0];
           max = scores[n-1];
           ...
           for(i = 0; i < sizeof(quantiles)/sizeof(*quantiles); i++) {
             ...
             if(min < max) {
               ...
               fprintf(fd, "%6.2f", r);
                ...
                // Modified removed the variable r piazza 477
               fprintf(fd, "***.***");
             }
             fprintf(fd, "\n");
           }
...
void reportquantiles(FILE *fd, Stats *s) {
        ...
        for(csp = s->cstats; csp != NULL; csp = csp->next) {
           ...
           for(fp = csp->freqs; fp != NULL; fp = fp->next) {
              ...
              i++;
              if(csp->tallied) {
                fprintf(fd, "(%6.2f,%6.2f)", fp->score, (float)fp->numless * 100.0/csp->tallied);
              } else {
                // modified
...
void reportmoments(FILE *fd, Stats *s) {
        ...
        for(csp = s->cstats; csp != NULL; csp = csp->next) {
           ...
           fprintf(fd, "Valid %3d; Mean %6.2f; Std. Dev. %6.2f\n",
                   csp->valid, (float)csp->mean, (float)csp->stddev);
           if(csp->sstats != NULL && csp->sstats->next == NULL)
                { continue; }  /* Only one section */
           for(ssp = csp->sstats; ssp != NULL; ssp = ssp->next) {
...
void reporthistos(FILE *fd, Course *c, Stats *s) {
        ...
        Student *stp = NULL;
        Freqs *fp = NULL;
        int col = 0, pct = 0, cnt = 0;
        int bins[50] = {0};
        float min = 0.0, max = 0.0, diff = 0.0;
...
// Modified removed int nm from parameter
