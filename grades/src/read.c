...
Course *readfile(char *root) {
    ...
    // dynamic
    tokentemp = malloc(64);
    tokenptr = tokenend = tokentemp;

    // // dynamic
    // fseek(ifile->fd, 0L, SEEK_END);
    // int new_size = ftell(ifile->fd);
    // rewind(ifile->fd);
    // // printf("####################%d\n",new_size); // 740
    // tokenptr = newstring(tokenend, new_size);
...
/*
 * Doubles the size of space for file each time you run out of space
 */
int doublesize(int size) {
    if(tokensize() > size) {
        if((tokentemp = realloc(tokentemp, size * 2))==NULL)
            fatal("Unable to allocate memory.");
        tokenptr = tokentemp;
        tokenend = tokentemp + (size * 2);
        return size * 2;
    }
    return size;
}
